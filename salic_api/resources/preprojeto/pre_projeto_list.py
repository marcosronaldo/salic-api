...
class PreProjetoList(ResourceBase):
    ...
    def get(self):
        ...
        try:
            ...
            results, n_records = PreProjetoModelObject().all(limit, offset, id,
                                                             nome,
                                                             data_inicio,
                                                             data_termino,
                                                             extra_fields=True)
        ...
        for preprojeto in data:
            ...
            preprojeto['impacto_ambiental'] = sanitize(
                preprojeto['impacto_ambiental'])
            preprojeto['especificacao_tecnica'] = sanitize(
                preprojeto['especificacao_tecnica'])
            preprojeto['estrategia_execucao'] = sanitize(
                preprojeto['estrategia_execucao'])
            preprojeto['democratizacao'] = sanitize(
                preprojeto["democratizacao"])

            preprojeto['sinopse'] = sanitize(
                preprojeto["sinopse"], truncated=False)
            preprojeto['resumo'] = sanitize(
                preprojeto["resumo"], truncated=False)

        self.build_links(
            args={'limit': limit, 'offset': offset, 'n_records': n_records})
